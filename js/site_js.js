...
function getItemCodeByParam(id, table, field, selector, qty_unit = '') {
    if (id) {
        ...
        // added this line for checking same material quantity: added a dynamic class:
        $('#quantity0').addClass('matclass_'+ id);
...
function getAppendItemCodeByParam(id, table, field, selector, qty_unit = '') {
    ...
    // added this line for checking same material quantity: added a dynamic class:
    $('#quantity' + id).addClass('matclass_' + materialId);
...
function check_stock_quantity_validation(selector_id) {
    ...
    if(selector_id == 0){
        var materialId = $('#material_name').val();
        var materialValueText = $("#material_name option:selected");
        var selectedMaterialText = materialValueText.text();
    }else{
        var materialId = $('#material_name' + selector_id).val();
        var materialValueText = $('#material_name' + selector_id +' option:selected');
        var selectedMaterialText = materialValueText.text();
    }
    var stockValue = 0;
    var quantityValue = 0;
    ...
    var commonIssueQuantityTotalValue   =   get_common_issue_quantity_total_value(materialId);
    if (stockValue < quantityValue) {
        $('#quantity' + selector_id).val('');
        show_stock_overflow_message(selectedMaterialText);
    }
...
function get_common_issue_quantity_total_value(materialId){

    var sumTotal = 0;
    $(".matclass_"+materialId).each(function(){
        sumTotal += +$(this).val();
    });
...
function show_stock_overflow_message(message){
    swal({
        title: '',
        html: true,
        text: message + " Stock Overflow",
